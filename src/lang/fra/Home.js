export default {
    banner: {
        info: {
            name: 'F: British-Ukrainian Agency International Agency of Ukraine',
            mainInfo: 'Our company works with Ukrainian universities. We want to provide you with a higher education in the center of Europe',
            button: 'Learn More'
        }
    },

    chooseUs: {
        title: 'Why you should choose education with us?',
        chooseItem1: {
            title: 'Many univercities',
            text: 'Our company is an intermediary between a university and a student. We will help solve your problems with life in Ukraine. And of course we will help to get an education. Our company has many people who know foreign languages. Such as: Ukrainian, Russian, English, French and Arabic.'
        },
        chooseItem2: {
            title: 'Good education',
            text: 'Our company is an intermediary between a university and a student. We will help solve your problems with life in Ukraine. And of course we will help to get an education. Our company has many people who know foreign languages. Such as: Ukrainian, Russian, English, French and Arabic.'
        },
        chooseItem3: {
            title: 'International company',
            text: 'Our company is an intermediary between a university and a student. We will help solve your problems with life in Ukraine. And of course we will help to get an education. Our company has many people who know foreign languages. Such as: Ukrainian, Russian, English, French and Arabic.'
        },
        chooseItem4: {
            title: 'Supporting our students',
            text: 'Our company is an intermediary between a university and a student. We will help solve your problems with life in Ukraine. And of course we will help to get an education. Our company has many people who know foreign languages. Such as: Ukrainian, Russian, English, French and Arabic.'
        }
    },

    ukraine: {
        title: 'Ukraine',
        text: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!',
        button: 'Learn More'
    },

    questions: {
        title: 'Frequently Asked Questions',
        desc: {
            part1: 'If you didn’t find what you were after, ',
            contact: 'contact',
            part2: 'us directly and we’ll  help.'
        },
        question1: {
            caption: 'What documents should I have for applying?',
            answer: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!'
        },
        question2: {
            caption: 'Question 2',
            answer: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!'
        },
        question3: {
            caption: 'Question 3',
            answer: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!'
        },
        question4: {
            caption: 'Question 4',
            answer: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!'
        },
        question5: {
            caption: 'Question 5',
            answer: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!'
        },
        question6: {
            caption: 'Question 6',
            answer: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab amet consectetur adipisicing elit. Laboriosam, aperiam a? Veniam dolorum, ut dignissimos ipsam explicabo quis enim veritatis dolor ex. Quasi commodi asperiores tenetur modi quisquam sequi ab!'
        }
    },

    navigation: {
        poltava: 'Poltava',
        ukraine: 'Ukraine',
        univercities: 'Univercities',
        cities: 'Cities',
        about: 'About us'
    }
}